...
namespace Installer.Core.Wim
{
    ...
    {
        private static XmlSerializer Serializer { get; } = new XmlSerializer(typeof(XmlModel.WimInfo));

        public WindowsImageInfo Load(Stream stream)
        {
            var metadata = (XmlModel.WimInfo) Serializer.Deserialize(GetWimMetadataStream(stream));

            return new WindowsImageInfo
            {
                Images = metadata.Images.Select(x => new ImageMetadata
                {
        ...
        private static MemoryStream GetWimMetadataStream(Stream wim)
        {
            ...
            using (var wimsecstream = wim)
            {
                using (var wimsecreader = new BinaryReader(wimsecstream))
                {
                    ...
                    Console.WriteLine("(wim) Finding Magic Bytes...");

                    var start = FindPosition(wimsecstream, bytes);

                    Console.WriteLine("(wim) Found Magic Bytes at " + start);

                    Console.WriteLine("(wim) Finding WIM XML Data...");
                    ...
                    Console.WriteLine("(wim) Found WIM XML Data at " + start + may + 2);
